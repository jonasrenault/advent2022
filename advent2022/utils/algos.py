...
def grid_find_adjacent(
    ...
    """
    Use breadth-first search to find and yield all the nodes reachable from src, returning
    each node in find along with its distance from src.
    ...
    Yields:
        Iterator[tuple[tuple[int, int], int]]: each target node reachable and its
        distance from src
    """
    visited = {src}
